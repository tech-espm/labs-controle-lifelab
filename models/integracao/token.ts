...
export = class Token {
	...
	private readonly credentialsJson: Buffer;
	...
	constructor(url: string, credentialsJson: string) {
		this.url = url;
		this.credentialsJson = Buffer.from(credentialsJson);
		this.header = null;
		this.validadeMS = 0;
	...
	private async gerar(): Promise<string> {
		const r = await app.request.json.postBuffer(this.url, this.credentialsJson, "application/json");

		if (r.success && r.result && r.result.token) {
			try {
				const token = r.result.token as string,
